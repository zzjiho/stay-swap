...
//    }
    ...
    public RestApiResponse<Slice<SwapListResponse>> getSwapListByRequester(
            @RequestParam("userId") Long userId,
        ...
        return RestApiResponse.success(swapListService.getSwapListByRequester(userId, swapStatus, swapType, pageable));
    }
    ...
    public RestApiResponse<Slice<SwapListResponse>> getSwapListByHost(
            @RequestParam("userId") Long userId,
            @RequestParam(value = "swapType", required = false) SwapType swapType,
            @RequestParam(value = "swapStatus", required = false) SwapStatus swapStatus,
            @PageableDefault(size = 10) Pageable pageable) {
        
        return RestApiResponse.success(swapListService.getSwapListByHost(userId, swapStatus, swapType, pageable));
    }
}